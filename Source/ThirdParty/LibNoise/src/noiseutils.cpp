...
namespace noise
{
  ...
  namespace utils
  {
    ...
    inline unsigned char BlendChannel (const unsigned char channel0,
      const unsigned char channel1, float alpha)
    {
    ...
    // hope.)
    inline unsigned char* UnpackFloat (unsigned char* bytes, float value)
    {
    ...
    inline unsigned char* UnpackLittle16 (unsigned char* bytes,
    ...
    inline unsigned char* UnpackLittle32 (unsigned char* bytes,
...
void WriterBMP::WriteDestFile ()
{
  ...
  try {
    pLineBuffer = new unsigned char[bufferSize];
  }
  catch (...) {
    throw noise::ExceptionOutOfMemory ();
  }
  ...
  os.write ("\0\0\0\0", 4);
  os.write ((char*)UnpackLittle32 (d, (unsigned int)BMP_HEADER_SIZE), 4);
  os.write ((char*)UnpackLittle32 (d, 40), 4);   // Palette offset
  os.write ((char*)UnpackLittle32 (d, (unsigned int)width ), 4);
  os.write ((char*)UnpackLittle32 (d, (unsigned int)height), 4);
  ...
  os.write ((char*)UnpackLittle32 (d, (unsigned int)destSize), 4);
  ...
  for (int y = 0; y < height; y++) {
    ...
    unsigned char* pDest   = pLineBuffer;
...
void WriterTER::WriteDestFile ()
{
  ...
  // Build the header.
  unsigned char d[4];
  short heightScale = (short)(floor (32768.0 / (double)m_metersPerPoint));
  ...
  for (int y = 0; y < height; y++) {
    ...
    unsigned char* pDest   = pLineBuffer;
    for (int x = 0; x < width; x++) {
      short scaledHeight = (short)(floor (*pSource * 2.0));
  ...
  const Color& backgroundColor, double lightValue) const
{
  ...
  // Rescale the color channels to the unsigned char (0..255) range and return
  // the new color.
  Color newColor (
    (unsigned char)((unsigned int)(red   * 255.0) & 0xff),
    (unsigned char)((unsigned int)(green * 255.0) & 0xff),
    (unsigned char)((unsigned int)(blue  * 255.0) & 0xff),
  ...
  double bumpHeight) const
{
  ...
  xc = (unsigned char)((unsigned int)((floor)((vxc + 1.0) * 127.5)) & 0xff);
  yc = (unsigned char)((unsigned int)((floor)((vyc + 1.0) * 127.5)) & 0xff);
  zc = (unsigned char)((unsigned int)((floor)((vzc + 1.0) * 127.5)) & 0xff);
